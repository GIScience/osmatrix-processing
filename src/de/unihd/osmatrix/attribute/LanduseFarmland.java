...
public class LanduseFarmland extends Attribute {
	 ...
	 */
	@Override
	protected double doUpdate(TABLE table, long cell_id, double area, ResultSet row) throws SQLException {
			String landuse = row.getString("landuse");
			if(landuse == null)
				return area;
			landuse = landuse.trim().toLowerCase();
			if(landuse.indexOf("farm") >= 0){
				if(values.add(landuse)){
					logger.info("Landuse_Farmland -> "+landuse);
				}
